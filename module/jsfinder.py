...
def find_by_url_deep(url):
    logger.info("start scan {}".format(url))
    html_raw = Extract_html(url)
    if html_raw == None:
        # print("Fail to access " + url)
        return None
        ...
        for temp_url in temp_urls:
            if temp_url not in urls:
                ...
                try:
                    ...
                    logger.info("add js error,{}".format(e))
                # urls.append(temp_url)
        i -= 1
    update_site(url, 1)
    logger.info("finish scan {}".format(url))
...
#     subdomains = find_subdomain(urls, domian)
    ...
    # if True:
    # with open('ohb.txt', "a", encoding='utf-8') as ff:
    #     ff.write(content_url)
